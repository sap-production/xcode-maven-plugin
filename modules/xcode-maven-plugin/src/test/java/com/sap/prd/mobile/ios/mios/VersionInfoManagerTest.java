...
public class VersionInfoManagerTest
{
  ...
  {
    Dependency dependency = VersionInfoManager.parseDependency(new File("src/test/resources/validXML.xml"));
    ...
    Assert.assertEquals("scm:perforce:MY_PERFORCE_PORT://MY_DEPOT_PATH", dependency.getScm().getConnection());
  ...
  public void testOldVersionInfoFormatAlpha20() throws Exception
  {
    ...
    dependencies.add(VersionInfoManager.parseDependency(new File("src/test/resources/versions-alpha-20.xml")));

    final ByteArrayOutputStream os = new ByteArrayOutputStream();

    try {

      new VersionInfoManager().createVersionInfoFile("com.sap.ondevice.production.ios.tests", "MyApp", "1.0.0",
  ...
  {
    List<Dependency> dependencies = new ArrayList<Dependency>();
    dependencies.add(VersionInfoManager.parseDependency(new File("src/test/resources/versions-beta-2.xml")));
  ...
  {
    List<Dependency> dependencies = new ArrayList<Dependency>();
    dependencies.add(VersionInfoManager.parseDependency(new File("src/test/resources/versions-beta-3.xml")));
